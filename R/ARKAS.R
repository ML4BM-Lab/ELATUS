...
#' @param kallisto_path Path to the Kallisto raw count matrix (As an example: kallisto_path=system.file("extdata", "kallisto_example_raw_matrix", package = "ARKAS"))
...
ARKAS <- function(kallisto_path, kallisto_name, cellRanger_path, organism, lower_emptydrops, EmptyDrops_FDR_thres, cells_mito_threshold, cells_max_threshold, cells_min_genes_detected_threshold, threshold_minumun_gene_counts, threshold_cells_detected, dimred_clustering, k_neighbors, ratio_threshold, CR_threshold, SI_threshold)
{
    ...
    candidate_lncRNAs <- get_candidates(kallisto_filt_sce, cellRanger_filt_sce , threshold_minumun_gene_counts = threshold_minumun_gene_counts, threshold_cells_detected = threshold_cells_detected,lncrna_names = lncrna_names,gtf=gtf)

    # clustering
    set.seed(100100100)
    kallisto_filt_sce <- scater::runPCA(kallisto_filt_sce) 
    k_neighbors = 5
    dimred_clustering = "PCA"
    ...
    candidate_lncRNAs$SI <- cell_type_specific_score[rownames(candidate_lncRNAs)]
    # To know in which cluster the SI is the highest 
    candidate_lncRNAs$cell_type_SI <- colnames(counts_cell_specificity_index[rownames(candidate_lncRNAs),])[apply(counts_cell_specificity_index[rownames(candidate_lncRNAs),],1,which.max)]
    ...
    CR_threshold = 10
    SI_threshold = 0.15
    biologically_relevant_lncRNAs <- biologically_relevant_lncRNAs(candidate_lncRNAs, ratio_threshold,CR_threshold,SI_threshold)
